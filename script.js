...
const CONFIG = {
  ...
  BRAND: 'AI Agency',
  PRIZE: 'BMW M760Li',
  SHEETS_URL: 'https://script.google.com/macros/s/AKfycbxdmOUrNrtiv_oQXceUE-1KwAjOQ-F2sJAWN0kpRPbYcobQ8pDGHtpXO0gdziPUY-O7/exec',
...
function renderTable(filter = '') {
  ...
  // Нормалізуємо телефон — лише останні 9 цифр
  function normalizePhone(phone) {
    return String(phone || '').replace(/\D/g, '').slice(-9);
  }

  // Групуємо тільки за нормалізованим телефоном
  const groups = [];
  const seen = {};
  all.forEach(p => {
    ...
    if (key && seen[key] !== undefined) {
      ...
      // Показуємо час останнього замовлення
      if (p.createdAt > groups[seen[key]].createdAt) {
        groups[seen[key]].createdAt = p.createdAt;
      }
    } else {
      if (key) seen[key] = groups.length;
...
function initCountdown() {
  ...
  function update() {
    ...
    if (heroLabel) heroLabel.textContent = '🏆 До розіграшу BMW M760Li — 25 серпня 2026';
