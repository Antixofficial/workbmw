...
async function handlePayment(formData) {
  ...
  // Зберігаємо двічі — основний і резервний ключ
  ...
  // Перевіряємо що дані справді збережені
  const check = localStorage.getItem('pendingOrder');
  if (!check) {
    showToast('❌ Помилка збереження. Спробуйте ще раз.');
    return;
  }

  // Невелика затримка щоб localStorage гарантовано зберігся
