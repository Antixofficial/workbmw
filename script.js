...
async function handlePayment(formData) {
  ...
  // 1. Зберігаємо в localStorage (для десктопу)
  try {
  ...
  // 2. Зберігаємо в Sheets як pending — надійний резервний канал для мобільних
  try {
    await fetch(CONFIG.SHEETS_URL, {
      method: 'POST',
      mode: 'no-cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'pending', ...pendingData }),
    });
  } catch(e) {}
