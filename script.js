...
if (!all.length) {
tbody.innerHTML = `<tr><td colspan="2" style="text-align:center;padding:40px;color:var(--muted)"> ${lang === 'uk' ? 'Покупців ще немає. Будь першим! 🚀' : 'No buyers yet. Be the first! 🚀'} </td></tr>`;
return;
}
...
container.innerHTML = items.map((item, i) => `<div class="faq-item" data-index="${i}"> <div class="faq-question"> <span>${esc(item.q)}</span> <div class="faq-chevron">▾</div> </div> <div class="faq-answer"><p>${esc(item.a)}</p></div> </div>`).join(’’);
...
overlay.style.cssText = `position: fixed; inset: 0; background: rgba(0,0,0,0.85); display: flex; align-items: center; justify-content: center; z-index: 99999; padding: 20px;`;
overlay.innerHTML = `<div style="background:#1a1a1a;border:1px solid rgba(255,255,255,0.1);border-radius:24px;padding:40px 32px;max-width:380px;width:100%;text-align:center;"> <div style="font-size:2.5rem;margin-bottom:16px">🔐</div> <h2 style="font-size:1.4rem;font-weight:800;margin-bottom:8px;color:#fff">Адмін панель</h2> <p style="color:#888;font-size:14px;margin-bottom:24px">Введіть пароль для доступу</p> <input type="password" id="adminPassInput" placeholder="Пароль..." style=" width:100%;background:#111;border:1px solid rgba(255,255,255,0.1); border-radius:12px;padding:14px 16px;color:#fff;font-size:15px; margin-bottom:12px;outline:none;box-sizing:border-box; " /> <button onclick="checkAdminPass()" style=" width:100%;background:linear-gradient(135deg,#ff6b35,#e07b39); border:none;border-radius:50px;padding:14px;color:#fff; font-size:15px;font-weight:700;cursor:pointer;margin-bottom:12px; ">Увійти</button> <button onclick="document.getElementById('adminLoginOverlay').remove();history.pushState('',document.title,window.location.pathname);" style=" background:none;border:none;color:#666;font-size:13px;cursor:pointer;text-decoration:underline; ">Скасувати</button> <div id="adminLoginError" style="color:#ff4444;font-size:13px;margin-top:8px;display:none">Невірний пароль</div> </div>`;
...
badge.id = ‘adminBadge’;
badge.style.cssText = `position: fixed; bottom: 20px; left: 20px; background: rgba(224,123,57,0.15); border: 1px solid rgba(224,123,57,0.4); border-radius: 50px; padding: 8px 16px; color: #e07b39; font-size: 13px; font-weight: 600; z-index: 9999; cursor: pointer;`;
