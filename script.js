...
function renderTable(filter = '') {
  ...
  tbody.innerHTML = all.map(p => `
    <tr>
      ...
      <td data-label=""><button class="delete-btn" onclick="handleDelete('${esc(p.number)}')">🗑</button></td>
    </tr>
  `).join('');
}
...
// HMAC-MD5 для WayForPay підпису
async function hmacMD5(key, data) {
  const enc = new TextEncoder();
  const cryptoKey = await crypto.subtle.importKey(
    'raw', enc.encode(key), { name: 'HMAC', hash: 'MD5' }, false, ['sign']
  );
...
async function handlePayment(formData) {
  ...
  const currency  = 'USD';
  const productName  = 'Ароматизатор для авто';
  const productPrice = amount;
  const productCount = 1;
  const merchant  = CONFIG.WAYFORPAY_MERCHANT;
  const domain    = 'bmwgiveaway.online';
  const secretKey = '614a8fb179e4671d26acccda2b7928f8f423cee5';

  // Рядок для підпису
  const signStr = [
    merchant, domain, orderId, orderDate,
    amount, currency, productName, productCount, productPrice
  ].join(';');

  const signature = await hmacMD5(secretKey, signStr);
  ...
  const wfpParams = {
    ...
    clientLastName:     formData.get('name')?.split(' ')[1] || '',
    clientEmail:        formData.get('email') || '',
    clientPhone:        formData.get('phone') || '',
    language:           'UA',
    returnUrl:          'https://bmwgiveaway.online/success.html',
    serviceUrl:         'https://bmwgiveaway.online/webhook',
  };

  if (btn) { btn.textContent = '💳 Оплатити та отримати номер'; btn.disabled = false; }

  // Зберігаємо дані форми до оплати
  const pendingData = {
  ...
  // Відкриваємо WayForPay
  const wfp = new Wayforpay();
  wfp.run(wfpParams,
    function onSuccess() { completePurchase(); },
    function onFail()    { showToast('❌ Оплату скасовано'); },
    function onError(e)  { showToast('❌ Помилка: ' + e); }
  );
}
